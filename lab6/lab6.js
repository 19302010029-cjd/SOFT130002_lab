...
function testTime(){
    ...
    else if(!flag) {console.log("已到达整分钟，停止翻倍");}
    else {console.log("已翻倍十次，停止翻倍");}
}
...
function testMail(telephone,mail) {
   let re1=RegExp(/^1\d{10}$/);
   let re2=RegExp(/^[a-zA-Z0-9]+[a-zA-Z0-9_.-]+@[a-zA-Z0-9_.-]+[.][a-zA-Z]{2,4}$/);
   if (re1.test(telephone)&&re2.test(mail)){
       console.log("The telephone is right and the mail is right.");
   }
   else if (!re1.test(telephone)&&re2.test(mail)){
...
function testKeyBoard(wantInput, actualInput) {
   let set=new Set();
   let str=wantInput+actualInput;
   let result="Set {";
   str=str.toUpperCase();
   for(let i=0;i<=str.length;i++){
       if (i<wantInput.length){
           set.add(str.charAt(i));
       }
       else{set.delete(str.charAt(i));}
   }
   set.forEach(function (value) {
       result+="'"+value+"', ";
   });
